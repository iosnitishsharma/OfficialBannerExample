 ...
 */
package example.billingmodule.billing;
...
/*
    Now we have the Play Billing API available in our project, we need to construct an instance of
    the BillingClient class. This can be instantiated by using the static newBuilder() method, passing in the reference to
    the activity which we are associating our Billing Client with. Finally, we can use the setListener() method to
    pass in a reference to our
    PurchasesUpdatedListener instance which is used to receive callbacks when the purchases for the current
    user are changed.*/
...
/*
   #3 Play Billing library provides the ability for us to retrieve a list of details for the items which
    are available for purchase.To do this we need to send a list of the SKUs which we wanted to retrieve the details for.
    We begin by using the SkuDetailsParams class to construct a new instance and setting the desired SKUs to our
    SKU list. At this point we also need to set the type of items we wish to be returned by the query — this
    will be either in-app purchase items or
    subscriptions. In the case of this query we’re after in-app purchases,
    so we’re going to use the INAPP SkuType.*/
  ...
  /* #2 Now that we have a reference to the Billing Client instance we can go ahead and kick off the connection to
    the in-app billing API. Because this request is asynchronous, we pass
    in an instance of a BillingClientStateListener so that we can receive events based on the status
